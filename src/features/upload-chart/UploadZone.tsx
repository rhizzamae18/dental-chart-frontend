...
import { uploadPage1, uploadPage2, uploadPage3, uploadPage4 } from '@/shared/lib/api';
...
const PAGE_LABELS = [
  ...
  { page: 2, title: 'Dental Chart & Clinical Findings', description: 'Tooth diagram, periodontal, occlusion, appliances, TMD' },
  { page: 3, title: 'Informed Consent & Treatment', description: 'Treatment consent, procedure consent, signatures' },
  { page: 4, title: 'Treatment Records', description: 'Treatment history, procedures, notes' },
];
...
function flattenExtractedData(data: any): Record<string, unknown> {
  ...
  if (data.dentalHistory) {
    ...
    const fullName = `${data.patient.lastName}, ${data.patient.firstName} ${data.patient.middleName || ''}`.trim();
    const age = data.patient.age;
    const gender = data.patient.sex === 'Male' || data.patient.sex === 'M' ? 'M' : 'F';
  ...
  if (data.medicalHistory) {
    Object.keys(data.medicalHistory).forEach(key => {
      if (key !== 'allergies' && key !== 'forWomenOnly' && key !== 'medicalConditions') {
        const mappedKey = fieldMapping[key] || key;
    ...
    if (data.medicalHistory.medicalConditions) {
      Object.keys(data.medicalHistory.medicalConditions).forEach(key => {
        flattened[`condition_${key}`] = data.medicalHistory.medicalConditions[key];
      });
    }
  }
  ...
  if (data.ToothFinding && Array.isArray(data.ToothFinding)) {
    ...
    data.ToothFinding.forEach((tooth: any) => {
      ...
      if (tooth.condition === 'MISSING_CARIES' || tooth.condition === 'MISSING_OTHER') conditions.missingTeeth.push(num);
      if (tooth.condition === 'IMPACTED') conditions.impactedTeeth.push(num);
      if (tooth.condition === 'ROOT_FRAGMENT') conditions.rootFragments.push(num);

      // Surgeries (Extractions)
      if (tooth.surgeries) {
        if (tooth.surgeries.includes('EXTRACTION_CARIES')) conditions.extractionCaries.push(num);
        if (tooth.surgeries.includes('EXTRACTION_OTHER')) conditions.extractionOther.push(num);
      }

      // Restorations
      if (tooth.restorations) {
        if (tooth.restorations.includes('AM')) restorations.amalgamFilling.push(num);
        if (tooth.restorations.includes('CO')) restorations.compositeFilling.push(num);
        if (tooth.restorations.includes('JC')) restorations.jacketCrown.push(num);
        if (tooth.restorations.includes('IN') || tooth.restorations.includes('IMP')) restorations.inlayImplant.push(num);
    ...
    Object.keys(conditions).forEach(key => {
      if (conditions[key].length > 0) flattened[key] = conditions[key].join(', ');
    });
    Object.keys(restorations).forEach(key => {
      if (restorations[key].length > 0) flattened[key] = restorations[key].join(', ');
    });
  }
  ...
  if (data.periodontal) {
    Object.keys(data.periodontal).forEach(key => {
      ...
      flattened[key] = val === true ? 'present' : (val === false ? 'absent' : 'unknown');
    });
  }

  // Occlusion
  if (data.occlusion) {
    if (data.occlusion.molarClass) flattened['occlusionClass'] = data.occlusion.molarClass; // Map molarClass -> occlusionClass
    if (data.occlusion.overjet) flattened['overjet'] = 'Present'; // Boolean -> Text
    if (data.occlusion.overbite) flattened['overbite'] = 'Present';
    if (data.occlusion.midlineDeviation) flattened['midlineDeviation'] = 'Present';
  ...
  if (data.appliances) {
    ...
    if (data.appliances.others) flattened['otherAppliances'] = data.appliances.others;
  }
  ...
  // Treatment Consent
  if (data.treatment?.initial) flattened['treatmentInitial'] = data.treatment.initial;

  // Procedures Consent
  if (data.drugsMedication?.initial) flattened['drugsInitial'] = data.drugsMedication.initial;
  if (data.changesInPlan?.initial) flattened['planChangesInitial'] = data.changesInPlan.initial;
  if (data.radiograph?.initial) flattened['radiographInitial'] = data.radiograph.initial;
  ...
  // Signatures
  if (data.patientSignature) flattened['patientSignature'] = data.patientSignature;
  if (data.dentistSignature) flattened['dentistSignature'] = data.dentistSignature;
  ...
  if (data.treatmentRecord && Array.isArray(data.treatmentRecord) && data.treatmentRecord.length > 0) {
    ...
    if (record.amountCharged) flattened['amountCharged'] = String(record.amountCharged);
    ...
    if (record.nextAppointment) flattened['nextAppointment'] = formatDate(record.nextAppointment);
  }
...
export const UploadZone = () => {
  ...
  const processAllPages = useCallback(async (pageImages: string[]) => {
    try {
      ...
      for (let i = 0; i < Math.min(pageImages.length, 4); i++) {
        ...
        const imageFile = new File([pageBlob], `page${pageNum}.png`, { type: 'image/png' });
        ...
        if (pageNum === 1) {
          ...
          result = await uploadPage1(imageFile);
          if (result.success && result.patientId) {
            currentPatientId = result.patientId;
            setPatientId(currentPatientId);
            console.log(`✅ Page 1 success! PatientId: ${currentPatientId}`);
          }
        } else if (pageNum === 2 && currentPatientId) {
          console.log(`📤 Uploading Page 2 with patientId: ${currentPatientId}...`);
          result = await uploadPage2(imageFile, currentPatientId);
          if (result.success && result.dentalChartId) {
            currentDentalChartId = result.dentalChartId;
            setDentalChartId(currentDentalChartId);
            console.log(`✅ Page 2 success! DentalChartId: ${currentDentalChartId}`);
          }
        } else if (pageNum === 3 && currentPatientId) {
          console.log(`📤 Uploading Page 3 with patientId: ${currentPatientId}...`);
          result = await uploadPage3(imageFile, currentPatientId);
          console.log(`✅ Page 3 ${result?.success ? 'success' : 'failed'}!`);
        } else if (pageNum === 4 && currentPatientId) {
          console.log(`📤 Uploading Page 4 with patientId: ${currentPatientId}...`);
          result = await uploadPage4(imageFile, currentPatientId);
          console.log(`✅ Page 4 ${result?.success ? 'success' : 'failed'}!`);
        }

        // Merge extracted data from this page
        if (result?.success && result.extractedData) {
          console.log(`📊 Merging data from Page ${pageNum}:`, result.extractedData);
          Object.assign(allExtractedData, result.extractedData);
        } else if (result && !result.success) {
          console.error(`❌ Page ${pageNum} extraction failed:`, result.error);
          // Continue to next page even if one fails
        }
      }

      console.log('🎉 All pages processed! Total extracted data:', allExtractedData);
  ...
  }, [setAppState, setExtractedData, setPatientId, setDentalChartId]);
  ...
  }, [localUploadMode, setUploadMode, setAppState, setSeparatedPages, processAllPages]);
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    if (localUploadMode === 'images') {
      ...
      if (imageFiles.length !== 4) {
        alert(`Please upload exactly 4 images for the complete dental chart (found ${imageFiles.length}).`);
        return;
      }
  ...
  }, [localUploadMode, setUploadMode, setAppState, setSeparatedPages, processAllPages, handleFile]);
    ...
    <div className="w-full sm:w-[90%] md:w-[80%] lg:w-[70%] mx-auto mt-6 sm:mt-10 space-y-6">
      ...
      <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-100">
        <h3 className="text-lg font-semibold text-primary mb-4 text-center">
          {localUploadMode === 'pdf' ? 'Your PDF should contain these 4 pages:' : 'Upload all 4 page images at once:'}
        </h3>
