...
export const generateDentalChartPDF = (
  ...
  const getData = (key: string) => {
    ...
    const fieldMap: Record<string, string[]> = {
      ...
      earlyPeriodontitis: ["earlyPeriodontitis"],
      moderatePeriodontitis: ["moderatePeriodontitis"],
      advancedPeriodontitis: ["advancedPeriodontitis"],

      // Informed Consent Initials
      treatmentInitial: ["treatmentInitial"],
      drugsInitial: ["drugsInitial"],
      planChangesInitial: ["planChangesInitial"],
      radiographInitial: ["radiographInitial"],
      removalInitial: ["removalInitial"],
      crownsInitial: ["crownsInitial"],
      rootCanalInitial: ["rootCanalInitial"],
      periodontalInitial: ["periodontalInitial"],
      fillingsInitial: ["fillingsInitial"],
      denturesInitial: ["denturesInitial"],
  ...
  doc.line(
    nameStartX + nameWidth1 + 3,
    yPos + 1,
    nameStartX + nameWidth1 + nameWidth2 + 3,
    yPos + 1
  );
  doc.line(
    nameStartX + nameWidth1 + nameWidth2 + 6,
    yPos + 1,
    pageWidth - margin,
    yPos + 1
  );
  ...
  doc.setFontSize(8);
  doc.text(
    "3. Have you ever been hospitalized or had serious illness or surgical operation?",
    margin,
    yPos
  );
  drawYesNoCheckbox(
  ...
  drawYesNoCheckbox(
    pageWidth - margin - 27,
    yPos - 2,
    isYes("takingMedication"),
    isNo("takingMedication")
  );
  yPos += 4;
  doc.setFontSize(7);
  ...
  drawParenCheckbox(
    ...
    isYes("allergyAnesthetic") || isYes("allergy_anesthetic"),
    col1X,
    yPos
  );
  drawParenCheckbox(
    "Penicillin, Antibiotics",
    isYes("allergyPenicillin") || isYes("allergy_penicillin"),
    col2X,
    yPos
  );
  yPos += 3.5;
  drawParenCheckbox(
    "Aspirin",
    isYes("allergyAspirin") || isYes("allergy_aspirin"),
    col1X,
    yPos
  );
  drawParenCheckbox(
    "Latex",
    isYes("allergyLatex") || isYes("allergy_latex"),
    col2X,
    yPos
  );
  yPos += 3.5;
  ...
  drawParenCheckbox(
    "Sulfa drugs",
    isYes("allergySulfa") || isYes("allergy_sulfa"),
    col3X,
    yPos
  );
  yPos += 3.5;
  drawParenCheckbox(
    "Others",
    isYes("allergyOthers") || isYes("allergy_others"),
    col3X,
    yPos
  );
  yPos += 5;
  ...
  const conditions = [
    {
      col1: "High Blood Pressure",
      col2: "Heart Disease",
      col3: "Cancer/Tumors",
    },
    { col1: "Low Blood Pressure", col2: "Heart Murmur", col3: "Anemia" },
    {
      col1: "Epilepsy/Convulsions",
      col2: "Hepatitis/Liver Disease",
      col3: "Angina",
    },
    { col1: "AIDS or HIV Infection", col2: "Rheumatic Fever", col3: "Asthma" },
    {
      col1: "Sexually Transmitted Disease",
      col2: "Hay Fever/Allergies",
      col3: "Emphysema",
    },
    {
      col1: "Stomach Troubles/Ulcers",
      col2: "Respiratory Problems",
      col3: "Bleeding Problems",
    },
    {
      col1: "Fainting Seizure",
      col2: "Hepatitis/Jaundice",
      col3: "Blood Diseases",
    },
    { col1: "Rapid Weight Loss", col2: "Tuberculosis", col3: "Head Injuries" },
    {
      col1: "Radiation Therapy",
      col2: "Swollen Ankles",
      col3: "Arthritis/Rheumatism",
    },
  ...
  conditions.forEach((row) => {
    if (row.col1) {
      const key1 =
        conditionKeyMap[row.col1] ||
        row.col1.replace(/[^a-zA-Z]/g, "").toLowerCase();
      drawParenCheckbox(
        row.col1,
        isYes(`condition_${key1}`) || isYes(key1),
        condCol1X,
        yPos
      );
    }
    if (row.col2) {
      const key2 =
        conditionKeyMap[row.col2] ||
        row.col2.replace(/[^a-zA-Z]/g, "").toLowerCase();
      drawParenCheckbox(
        row.col2,
        isYes(`condition_${key2}`) || isYes(key2),
        condCol2X,
        yPos
      );
    }
    if (row.col3) {
      const key3 =
        conditionKeyMap[row.col3] ||
        row.col3.replace(/[^a-zA-Z]/g, "").toLowerCase();
      drawParenCheckbox(
        row.col3,
        isYes(`condition_${key3}`) || isYes(key3),
        condCol3X,
        yPos
      );
    }
  ...
  doc.setFontSize(6.5);
  doc.text(patientDate || "", margin + 85 + 22.5, yPos - 1.5, {
    align: "center",
  });
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text("Page 3 of 5", pageWidth / 2, pageHeight - 10, { align: "right" });
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  const fullName = `${getData("firstName")} ${getData("middleName")} ${getData(
    "lastName"
  )}`.trim();
  ...
  console.log(
    getData("gingivitis"),
    getData("earlyPeriodontitis"),
    getData("moderatePeriodontitis"),
    getData("advancedPeriodontitis")
  );
  ...
  perioItems.forEach(({ label, key }) => {
    const value = getData(key);

    doc.rect(margin, yPos - 2.2, 2, 2);

    if (value === "present") {
      doc.setFontSize(7);
      doc.text("X", margin + 0.45, yPos - 0.4);
      doc.setFontSize(6.5);
    }
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(0, 0, 0);
  const patientFullName = `${getData("lastName") || ""}, ${
    getData("firstName") || ""
  }`;
  ...
  if (hasFormTreatmentData) {
    ...
    const row = [
      ...
      formData.procedure || "",
      formData.dentistName || "",
      formData.amountCharged !== undefined
        ? String(formData.amountCharged)
        : "",
    ...
    const treatmentRecords =
      ...
      extractedData.treatmentRecord ||
      extractedData.treatmentRecords ||
      [];
    ...
    if (Array.isArray(treatmentRecords) && treatmentRecords.length > 0) {
      treatmentRecords.forEach((record: any) => {
        const row = [
          ...
          record.dentist || "",
          record.amountCharged !== undefined && record.amountCharged !== null
            ? String(record.amountCharged)
            : "",
          record.amountPaid !== undefined && record.amountPaid !== null
      ...
      console.log("No treatment records found in data");
    }
  }
  ...
  const patientName = patientFirstName
    ? `${patientLastName}_${patientFirstName}`
    : patientLastName;
